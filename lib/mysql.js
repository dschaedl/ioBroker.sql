...
exports.init = function () {
    return [
        ...
        "CREATE TABLE iobroker.ts_number  (id INTEGER, dt datetime, ts BIGINT, val REAL,    ack BOOLEAN, _from INTEGER, q INTEGER, PRIMARY KEY(id, ts));",
        "CREATE TABLE iobroker.ts_string  (id INTEGER, dt datetime, ts BIGINT, val TEXT,    ack BOOLEAN, _from INTEGER, q INTEGER, PRIMARY KEY(id, ts));",
        "CREATE TABLE iobroker.ts_bool    (id INTEGER, dt datetime, ts BIGINT, val BOOLEAN, ack BOOLEAN, _from INTEGER, q INTEGER, PRIMARY KEY(id, ts));",
        "ALTER TABLE iobroker.ts_number ADD INDEX 'ts_number_id' ('id' ASC)",
        "ALTER TABLE iobroker.ts_number ADD INDEX 'ts_number_ts' ('ts' ASC)",
        "ALTER TABLE iobroker.ts_bool ADD INDEX 'ts_bool_id' ('id' ASC)",
        "ALTER TABLE iobroker.ts_bool ADD INDEX 'ts_bool_ts' ('ts' ASC)",
        "ALTER TABLE iobroker.ts_string ADD INDEX 'ts_string_id' ('id' ASC)",
        "ALTER TABLE iobroker.ts_string ADD INDEX 'ts_string_ts' ('ts' ASC)"
    ];
};
...
exports.insert = function (index, state, from, db) {
    ...
    return "INSERT INTO iobroker." + db + " (id, ts, val, ack, _from, q, dt) VALUES(" + index + ", " + state.ts + ", " + state.val + ", " + (state.ack ? 1 : 0) + ", " + (from || 0) + ", "
        + state.q + ", '" + dateFormat(state.ts*1000, 'yyyy-mm-dd HH:MM:ss') + "');";
};
