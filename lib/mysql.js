...
exports.insert = function (index, state, from, db) {
    ...
    return "INSERT INTO iobroker." + db + " (id, ts, val, ack, _from, q) VALUES(" + index + ", " + state.ts + ", " + state.val + ", " + (state.ack ? 1 : 0) + ", " + (from || 0) + ", "
        + state.q + ", '" + dateFormat(state.ts, 'yyyy-mm-dd HH:MM:ss') + "');";
};
